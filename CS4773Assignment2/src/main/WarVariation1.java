...
public class WarVariation1 implements GameOfWar{
	Player player1;
	Player player2;
	...
	public void resolveRound(Player roundWinner) {
		...
		wonCards.clear();
		roundWinner.updateScore();
		roundWinner.getPlayerWinPile().refillPlayerHand(roundWinner.getCurrentHand());
	}
	...
	public void playGame() {
		...
		while(gameInProgress) {
			...
			logger.info("Score is " + player1 + " " + player1.getScore() + ", " + player2 + " " + player2.getScore());
			if ((player1.getCurrentHand().getCardsLeft()  == 0) || (player2.getCurrentHand().getCardsLeft()  == 0)){
